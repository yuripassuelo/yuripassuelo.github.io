...
tic
[V,gk,gl,stationary_dist,w,Kd,Ks,Ls,r,excDem] = ModelSolution(alpha,beta,gamma,theta,eta,delta,l_grid,n_grid,k_grid,pi,N,M,L,Nbar,V);
toc
...
% Plot das Funções Valores
plot( V )
title( "$\mathbf{Funcao\ Valor}$", 'Interpreter', 'latex' )
ylabel( "Utilidade", "Interpreter", "latex")
xlabel( "$k$", 'Interpreter', 'latex' )
legend( '$\eta = 0.5$', '$\eta = 1.0$', '$\eta = 1.1$', 'Interpreter', 'latex', 'Location', 'northwest' )

% Plot das Funções Politícas (Capital)
plot( gk )
title( "$\mathbf{Funcao\ Politica\ do\ Capital}$", 'Interpreter', 'latex' )
ylabel( "$k'$", "Interpreter", "latex")
xlabel( "$k$", 'Interpreter', 'latex' )
legend( '$\eta = 0.5$', '$\eta = 1.0$', '$\eta = 1.1$', 'Interpreter', 'latex', 'Location', 'northwest' )

% Plot das Funções Politícas (Trabalho)
plot( gl )
title( "$\mathbf{Funcao\ Politica\ do\ Trabalho}$", 'Interpreter', 'latex' )
ylabel( "$l$", "Interpreter", "latex")
xlabel( "$k$", 'Interpreter', 'latex' )
legend( '$\eta = 0.5$', '$\eta = 1.0$', '$\eta = 1.1$', 'Interpreter', 'latex', 'Location', 'northeast' )

% Distribuição Agregada

plot( stationary_dist )
title( "$\mathbf{Distribuicao\ Estacionaria}$", 'Interpreter', 'latex' )
ylabel( "Densidade", "Interpreter", "latex")
xlabel( "$k$", 'Interpreter', 'latex' )
legend( '$\eta = 0.5$', '$\eta = 1.0$', '$\eta = 1.1$', 'Interpreter', 'latex', 'Location', 'northeast' )
