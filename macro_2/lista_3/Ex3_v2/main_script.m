...
% Taxação Igual de Capital e Trabalho
tic
[Vb,gkb,gcb,stationary_distb,wb,Kdb,r0b,t0b,excDemb,GDPb ] = ModelSolution(alpha,beta,gamma,delta,n_grid,k_grid,pi,N,M,Nbar,V,'b',0.1);
toc

% Taxação Apenas de Capital
tic
[Vk,gkk,gck,stationary_distk,wk,Kdk,r0k,t0k,excDemk,GDPk ] = ModelSolution(alpha,beta,gamma,delta,n_grid,k_grid,pi,N,M,Nbar,V,'k',0.1);
toc

% Taxação Apenas de Trabalho
tic
[Vl,gkl,gcl,stationary_distl,wl,Kdl,r0l,t0l,excDeml,GDPl ] = ModelSolution(alpha,beta,gamma,delta,n_grid,k_grid,pi,N,M,Nbar,V,'l',0.1);
toc


%% Trajetória de Transição
% Começamos com taxação sobre salarios (w)
% Mantemos o gasto constante
% Mudamos a taxação para o capital (r)

% Setando parâmetros

% Períodos de transiçaõ
T = 20;

% Chute da transição do Capital agregado
% Aqui nosso chute será o KdK (K de S.S. com novo imposto)

Ktrans = Kdk + zeros(1,T);
